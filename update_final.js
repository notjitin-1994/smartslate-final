...
// 1. Fix Status Chips in Why Solara header
content = content.replace(
  /sx=\{\{ bgcolor: '#10b981', color: '#fff', fontWeight: 800, borderRadius: '4px' \}\}/g,
  "sx={{ bgcolor: '#a7dadb', color: '#091521', fontWeight: 800, borderRadius: '4px' }}"
);
content = content.replace(
  /sx=\{\{ bgcolor: '#4F46E5', color: '#fff', fontWeight: 800, borderRadius: '4px' \}\}/g,
  "sx={{ bgcolor: '#a7dadb', color: '#091521', fontWeight: 800, borderRadius: '4px' }}"
);
content = content.replace(
  /sx=\{\{ bgcolor: '#f59e0b', color: '#fff', fontWeight: 800, borderRadius: '4px' \}\}/g,
  "sx={{ bgcolor: '#a7dadb', color: '#091521', fontWeight: 800, borderRadius: '4px' }}"
);

// 2. Unify Grid Card Badges and Fix Spectrum Link
...
if (startIndex !== -1 && tempEndIndex !== -1) {
  ...
  const unifiedGrid = `{/* Solara Ecosystem Grid */}
  ...
  content = content.substring(0, startIndex) + unifiedGrid + content.substring(endIndex);
}

// 3. Global Padding Match Hero
content = content.replace(/py: \{ xs: 12, md: 16 \}/g, "pt: { xs: 8, md: 12 }, pb: { xs: 12, md: 16 }");
content = content.replace(/py: 'var\(--section-padding-y\)'/g, "pt: { xs: 8, md: 12 }, pb: { xs: 12, md: 16 }");

fs.writeFileSync(filePath, content);
console.log('Final polish and consistency applied');
