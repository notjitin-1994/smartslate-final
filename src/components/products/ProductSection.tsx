...
export default function ProductSection({ product, reverse = false, children }: ProductSectionProps) {
  ...
  return (
    <>
      {/* Intersection observer sentinel to avoid ref conflicts across responsive sections */}
      <div ref={ref} className="h-px w-px opacity-0 pointer-events-none" />

      {/* Mobile: overlay text on top of animated visual background with subtle blur */}
      <motion.section
        initial={{ opacity: 0, y: 50 }}
        animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 50 }}
        transition={{ duration: 0.8 }}
        className="md:hidden relative min-h-[560px]"
      >
        {/* Visual background */}
        <div className="absolute inset-0 -z-10 overflow-hidden flex items-center justify-center">
          <div className="absolute inset-0 pointer-events-none" style={{ background: 'linear-gradient(to bottom, rgba(2,12,27,0.35), rgba(2,12,27,0.08) 46%, rgba(2,12,27,0.45))' }} />
        ...
        {/* Text overlay with aesthetic blur */}
          ...
          <div className="glass-effect-strong rounded-2xl p-6">
            {product.status === 'coming-soon' && (
              <motion.span
                ...
                className="inline-block px-4 py-1 text-sm font-medium text-primary-accent bg-primary-accent/10 rounded-full border border-primary-accent/20 mb-4"
              >
                Coming Soon
              </motion.span>
            )}
            ...
            >
              <h2 className="text-3xl font-bold mb-3">{product.heading}</h2>
              <p className="text-lg text-primary-accent font-medium mb-4">{product.tagline}</p>
      ...
      <motion.section
        ...
        animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 50 }}
        transition={{ duration: 0.8 }}
        className={`hidden md:grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center ${
